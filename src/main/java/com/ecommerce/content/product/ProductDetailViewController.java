...
public class ProductDetailViewController implements MainLayoutController.MainLayoutAware {
    ...
    private void refreshUI(Product product) {
        ...
        // Format harga ke dalam format Indonesia (Rupiah)
        NumberFormat currencyFormat = NumberFormat.getCurrencyInstance(new Locale("id", "ID"));
        String formattedPrice = currencyFormat.format(product.getPrice());
        productPriceLabel.setText(formattedPrice);
        ...
        try {
            ...
            productImageView.setImage(new Image(getClass().getResource("/images/products/default.jpg").toExternalForm()));
        }
    }
    ...
    private void handleBuyNow() {
        ...
        if (App.loggedInUser == null) {
            ...
            alert.setHeaderText("Silakan Login");
            alert.setContentText("Anda harus login untuk melakukan pembelian.");
            alert.showAndWait();
            return;
        }
        ...
        try {
            ...
            String stockQuery = "SELECT stock, seller_id FROM products WHERE id = ?";
            ...
            String insertOrderQuery = "INSERT INTO orders (user_id, total_price, status, created_at, updated_at) VALUES (?, ?, ?, NOW(), NOW())";
            int orderId = 0;
            ...
            try (PreparedStatement orderStmt = connection.prepareStatement(insertOrderQuery, Statement.RETURN_GENERATED_KEYS)) {
                ...
                try (ResultSet generatedKeys = orderStmt.getGeneratedKeys()) {
                    if (generatedKeys.next()) {
                        ...
                        throw new SQLException("Gagal mendapatkan ID order.");
                    }
                }
            }

            // Tambahkan data ke tabel order_sellers
            String insertOrderSellerQuery = "INSERT INTO order_sellers (order_id, seller_id, status, created_at, updated_at) VALUES (?, ?, ?, NOW(), NOW())";
            int orderSellerId = 0;

            try (PreparedStatement orderSellerStmt = connection.prepareStatement(insertOrderSellerQuery, Statement.RETURN_GENERATED_KEYS)) {
                orderSellerStmt.setInt(1, orderId);
                orderSellerStmt.setInt(2, sellerId);
                orderSellerStmt.setString(3, "PENDING");
                orderSellerStmt.executeUpdate();

                try (ResultSet generatedKeys = orderSellerStmt.getGeneratedKeys()) {
                    if (generatedKeys.next()) {
                        orderSellerId = generatedKeys.getInt(1);
                    } else {
                        throw new SQLException("Gagal mendapatkan ID order_seller.");
                    }
                }
            }

            // Tambahkan data ke tabel order_items
            String insertOrderItemQuery = "INSERT INTO order_items (order_seller_id, product_id, quantity, price, created_at, updated_at) VALUES (?, ?, ?, ?, NOW(), NOW())";

            try (PreparedStatement orderItemStmt = connection.prepareStatement(insertOrderItemQuery)) {
                orderItemStmt.setInt(1, orderSellerId);
                orderItemStmt.setInt(2, productId);
                orderItemStmt.setInt(3, quantity);
                orderItemStmt.setDouble(4, productPrice);
            ...
            alert.setContentText("Terima kasih telah melakukan pembelian. Pesanan Anda sedang diproses.");
            alert.showAndWait();

            // Navigasi ke halaman CheckoutView
            String checkoutViewPath = "/com/ecommerce/content/customer/CheckoutView.fxml";
            App.mainLayoutController.loadContent(checkoutViewPath);

        } catch (Exception e) {
            System.err.println("[ERROR] Terjadi kesalahan saat memproses pesanan:");
            e.printStackTrace();
            ...
            if (connection != null) {
                try {
                    ...
                    System.err.println("[ERROR] Gagal melakukan rollback:");
            ...
            if (connection != null) {
                try {
                    ...
                    System.err.println("[ERROR] Gagal menutup koneksi database:");
        ...
        try (Connection connection = DatabaseUtils.getConnection()) {
            ...
            try (PreparedStatement statement = connection.prepareStatement(query)) {
                ...
                if (rs.next()) {
                    ...
                    return new Product(rs.getInt("id"), rs.getString("name"), rs.getDouble("price"), rs.getString("image_url"), rs.getString("description"), rs.getInt("stock"));
                } else {
    ...
    private void handleAddToCart() {
        ...
        if (App.loggedInUser == null) {
            ...
            String loginViewPath = "/com/ecommerce/content/LoginView.fxml";
            if (navbarController != null) {
                navbarController.addPageToStack(loginViewPath, Map.of());
            }
            App.mainLayoutController.loadContent(loginViewPath);
            return;
        }
        ...
        try (Connection connection = DatabaseUtils.getConnection()) {
            ...
            if (navbarController != null) {
                navbarController.addPageToStack(cartViewPath, Map.of("productName", productNameLabel.getText(), "productPrice", String.valueOf(productPrice), "quantity", String.valueOf(quantity)));
            }
